...
func AddAddress() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		math_filter := bson.D{{Key: "$match", Value: bson.D{primitive.E{Key: "_id", Value: address}}}}
		unwind := bson.D{{Key: "$unwind", Value: bson.D{primitive.E{Key: "path", Value: "$address"}}}}
		group := bson.D{{Key: "$group", Value: bson.D{primitive.E{Key: "_id", Value: "$address_id"}, {Key: "count", Value: bson.D{primitive.E{Key: "$sum", Value: 1}}}}}}
		pointcursor, err := UserCollection.Aggregate(ctx, mongo.Pipeline{math_filter, unwind, group})
		if err != nil {
			c.IndentedJSON(500, "Internal Server Error")
		}
...
func DeleteAddress() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if user_id == "" {
			c.Header("Content-Type", "application/json")
			c.JSON(http.StatusNotFound, gin.H{"Error": "Invalid search indedx"})
			c.Abort()
			return
		}
		...
		_, err = UserCollection.UpdateOne(ctx, filter, update)
		if err != nil {
			c.IndentedJSON(404, "Cannot update")
			return
		}
		defer cancel()
		ctx.Done()
